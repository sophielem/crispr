...
cat ./fail.log;
    ...
    echo python -u $CRISPR_TOOL_SCRIPT_PATH/specific_gene.py -f $fileSet -sl 20 -pam "NGG" -gi "$gi" -gni "$gni" -r "$URL_CRISPR"  -c 2000 --no-proxy -blast $parseBlast >> sg.cmd
    python -u $CRISPR_TOOL_SCRIPT_PATH/specific_gene.py -f $fileSet -sl 20 -pam "NGG" -gi "$gi" -gni "$gni" -r "$URL_CRISPR"  -c 2000 --no-proxy -blast $parseBlast 2>> ./specific_gene.err 1> ./specific_gene.log
